...
Use [docs/sandbox_process/README.md](docs/sandbox_process/README.md) for package
docs, [tests/README.md](tests/README.md) for test-tree layout, and
[docs/sandbox_process/verification/README.md](docs/sandbox_process/verification/README.md)
...
```bash
uv run pytest tests/sandbox_process
```

Run only hermetic tests:

```bash
uv run pytest tests/sandbox_process -m hermetic
```
...
```bash
direnv exec . uv run python examples/sandbox_process/<module>.py
```

Keep examples focused on imports from `sandbox_process`. If an example
...
```bash
direnv exec . uv run python -m workbench.sandbox_process.<module>
```
...
direnv exec . uv run py-lib-reproduce-running-loop \
    workbench.sandbox_process.<module>
```
