...
description: High-level system story for sandbox process. Use when you need the integrated architecture narrative before reading focused docs.
---
...
Sandbox Process starts with the common py library foundation: callers
import supported names from `sandbox_process`; `_api/config.py` re-exports
...
```mermaid
flowchart LR
    Caller["Caller"] --> Public["sandbox_process"]
...
The baseline package exports `__version__`, `SampleLibError`,
`InvalidConfigValueError`, `SampleLibConfig`, `get_config`, and
